...
public:
    ...
    */
    ...
    explicit BudgetManipulation(string quotaName)
    {
        ...
        //Debug code that lists all the transactions
        for (int j = 0; j < transactionVec.size(); j++) {
            cout << transactionVec[j].getCategory() << " | " << transactionVec[j].getName()
                 << " | " << transactionVec[j].getAmount() << " | " << transactionVec[j].getDate().toString() << endl;
        }
        this->quotaVec = myBudget.getAllQuotas();
        //Debug code that lists all the quotas
        for (int i = 0; i < 9; i++) {
            cout << quotaVec[i].getCategory() << " | " << quotaVec[i].getSpendLimit() << endl;
        }
        setCategories();
        setQuotas();
        setExpenses();
